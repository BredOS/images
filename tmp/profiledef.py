...
cmdline = "rw console=tty0 console=ttyS0,115200 earlycon rootwait stmmaceth=chain_mode:1 rootflags=subvol=@ rootfstype=btrfs"
...
U_BOOT_PARAMETERS="console=ttyS0,115200 console=tty1 console=both rootwait rw init=/sbin/init rootflags=subvol=@ rootfstype=btrfs"
#U_BOOT_ROOT=""
#U_BOOT_TIMEOUT="50"

# Device tree file (will be searched and copied to /boot if U_BOOT_COPY_DTB_TO_BOOT=true)
U_BOOT_FDT="jh7110-milkv-mars.dtb"
...
partition_table = lambda img_size, fs: {
    ...
    "boot": ["32M", "200M", "168M", "fat32"],
    "primary": ["200M", "100%", str(int(img_size / 1000) - 150) + "M", fs],
}

partition_suffix = lambda config_dir, disk: [
    ["mkfs.fat", "-v", "-F32", "-n", "BOOT", disk + "p1"],
    # ["dd", "if=" + config_dir + "bootinfo_sd.bin", "of=" + disk, "seek=0", "conv=notrunc"],
    # ["dd", "if=" + config_dir + "FSBL.bin", "of=" + disk, "seek=256", "conv=notrunc"],
    # ["dd", "if=" + config_dir + "u-boot-env-default.bin", "of=" + disk, "seek=768", "conv=notrunc"],
    # ["dd", "if=" + config_dir + "u-boot-opensbi.itb", "of=" + disk, "seek=1664", "conv=notrunc"],
]
